...
open class MapView: UIView {
    ...
    @available(iOS, deprecated: 17.5, message: "Use init(frame:mapInitOptions:urlOpener:) instead")
    ...
    private func subscribeToLifecycleNotifications() {
        if #available(iOS 17.5, *), bundle.infoDictionary?["UIApplicationSceneManifest"] != nil {
            notificationCenter.addObserver(self,
    ...
    private func checkForMetalSupport() {
        ...
        guard ProcessInfo().isOperatingSystemAtLeast(OperatingSystemVersion(majorVersion: Int(17.5), minorVersion: 0, patchVersion: 0)) else {
