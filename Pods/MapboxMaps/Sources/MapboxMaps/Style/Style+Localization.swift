...
extension Style {
    ...
    internal func getLocaleValue(locale: Locale) -> String? {
        ...
        if let languageCode = locale.language.languageCode {
            if supportedLocaleIdentifiers.contains(locale.language.languageCode?.identifier ?? <#default value#>) {
                return nil
            }
        }
        ...
        for sourceInfo in vectorSources {
            if locale.identifier.starts(with: "zh") {
                ...
                if let url = vectorSource.url {
                    if url.contains("mapbox.mapbox-streets-v7") {
                        if locale.identifier == "zh-Hant" {
                            return "zh"
                        }
                    } else if url.contains("mapbox.mapbox-streets-v8") {
        ...
        return supportedLocaleIdentifiers.contains(locale.identifier) ? locale.identifier : locale.language.languageCode?.identifier
    }
